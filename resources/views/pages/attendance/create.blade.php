    ...
    <form method="get" class="w-fit flex flex-row items-center justify-between mb-4 gap-2">
        <div class="flex flex-1 gap-2">
            ...
            <select id="class" name="class" onchange="this.form.submit()"
                class="bg-gray-50 w-44 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                <option value="">Pilih Kelas</option>
                @foreach ($classrooms as $class)
                    <option value="{{ $class->id }}" {{ request('class') == $class->id || $loop->first ? 'selected' : '' }}>
                        {{ $class->name }}
                    </option>
                @endforeach
            </select>
